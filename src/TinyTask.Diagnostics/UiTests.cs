...
internal static class UiTests
{
    ...
    internal static async Task<int> Run(string report)
    {
        ...
        try
        {
            ...
            try {wizard.Show();Check("Repeated test clicks register without claiming isolation",await wizard.TestRepeatedClicks());Snapshot(wizard,report,"wizard-dark");Check("Only assigned mouse affects test cursor",wizard.TestDeviceFilter());}finally{wizard.Close();}
